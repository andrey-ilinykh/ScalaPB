...
class DescriptorImplicits(params: GeneratorParams, files: Seq[FileDescriptor]) {
  ...
  implicit class EnumDescriptorPimp(val enumDescriptor: EnumDescriptor) {
    ...
    def scalaDescriptorSource: String =
      ...
      else s"${enumDescriptor.getContainingType.scalaType.fullName}.scalaDescriptor.enums(${enumDescriptor.getIndex})"
    ...
    def sourcePath: Seq[Int] = {
      if (enumDescriptor.isTopLevel) Seq(FileDescriptorProto.ENUM_TYPE_FIELD_NUMBER, enumDescriptor.getIndex)
      else
